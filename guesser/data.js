...
function normalizeWeapon(raw) {
  ...
  return {
    ...
    enemyMaterial: raw.enemyMaterial, // （こちらは通常の雑魚敵素材）
...
// ---------------------------------------------------------------------------
// ===========================================================================
// ここからファイルの最下部まで、すべて上書き・置き換えしてください
// ===========================================================================
...
// 1. まず、グループ分け用の箱「weaponGroups」を最優先で作成・初期化します
...
const WEAPON_ENEMY_MATERIALS_MAP = {
  ...
  "斬山の刃": "猟兵の刀", "黒岩の長剣": "猟兵の刀", "匣中龍吟": "猟兵の刀", "暗鉄剣": "猟兵の刀", "千岩古剣": "猟兵の刀", "白影の剣": "猟兵 of 刀", "和璞鳶": "猟兵の刀", "流月の針": "猟兵の刀", "白纓槍": "猟兵の刀", "黒岩の緋玉": "猟兵の刀", "匣中日月": "猟兵の刀", "翡玉法珠": "猟兵の刀", "落霞": "猟兵の刀", "黒岩の戦弓": "猟兵の刀", "弓蔵": "猟兵の刀", "弾弓": "猟兵の刀",
  ...
  "砂中の賢者達の問答": "翼状の羽鰭", "鶴鳴の余韻": "翼状の羽鰭", "白雨心弦": "翼状 of 羽鰭", "チェーンブレイカー": "翼状の羽鰭",
...
// 3. 武器データを1つずつ成形（正規化）する関数
function normalizeWeapon(raw) {
  const aliases = Object.keys(WEAPON_NAME_ALIASES).filter(legacy => WEAPON_NAME_ALIASES[legacy] === raw.name);
  
  // ★ マッピング表からエリート敵素材の名前を安全に自動取得
  const weaponEnemyMaterial = WEAPON_ENEMY_MATERIALS_MAP[raw.name] || ''; 
...
// 4. グループ分けしたデータをもとに、成形済みの武器リストを生成します（ここで上のすべてが初期化されている必要があります）
...
const WEAPON_HINT_FIELDS = [
  ...
  { key: 'weaponType',             label: '武器種',       type: 'exact',   onDefault: true },
