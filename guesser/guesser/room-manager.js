(function (root, factory) {
  ...
  const logic = root.GenshinGameLogic || (typeof GenshinGameLogic !== 'undefined' ? GenshinGameLogic : null);
  root.RoomManager = factory(logic).RoomManager;
  root.ALLOWED_STAMPS = factory(logic).ALLOWED_STAMPS;
  root.DEFAULT_RULES = factory(logic).DEFAULT_RULES;
})(typeof globalThis !== 'undefined' ? globalThis : this, function (logicModule) {
  'use strict';

  const logic = logicModule || (typeof GenshinGameLogic !== 'undefined' ? GenshinGameLogic : {});
  const getPool = logic.getPool || (() => []);
  const getHintFields = logic.getHintFields || (() => []);
  const findItemById = logic.findItemById || (() => null);
  const compareItem = logic.compareItem || (() => ({}));
  ...
  class RoomManager {
    ...
    generateRoomCode() {
      ...
      return generateUUID().replace(/-/g, '').slice(0, 6).toUpperCase();
    }
